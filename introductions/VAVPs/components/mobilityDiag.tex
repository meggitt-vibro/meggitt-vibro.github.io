...
%		\end{scope}
		...
		\node[outer sep=0pt,thick] at (2.25,1.4) [] {{\color{blue}${{f}}_{j}$}};
		
		\draw [line width=1.5pt,red,-latex]  (5.2,-4.8) -- ++( -0.15, 1)  ;
		\node[outer sep=0pt,thick] at (5.5,-4.2) [] {{\color{red}${{v}}_{i}$}};
		
		
		\draw [line width=1pt,dashed,-latex]  plot[smooth, tension=.6] coordinates {(2.2,0.15) (3.2,-1.5) (4,-3.5)  (5.2,-4.8) }; 
		\node[outer sep=0pt,thick] at (2.9,-2.2) [] {{${Y}_{ij}$}};
		...
		\begin{scope}[xshift=5.2cm,yshift=-4.8cm]
			\node[outer sep=0pt,thick] at (0, 0)  [yshift=-0.3cm] {\footnotesize{\color{red}$f_{k\neq j}=0$}};
		\end{scope}
