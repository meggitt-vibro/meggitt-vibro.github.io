...
\begin{document}
	\begin{tikzpicture}[scale=0.85,every node/.style={outer sep=0pt,transform shape},thick,
		...
		%% Internal forces [yshift=1cm]m1.west
		\begin{scope}

		\draw[<-,red] ($(m1.west)-(0.1,2.5)$) -- ($(m1.west)-(1,2.5)$) node[midway,below=1mm]{$f_1$};  
	
		\draw[<-,red] ($(m2.west)-(0.1,1.65)$) -- ($(m2.west)-(1,1.65)$) 	node[midway,below=1mm]{$f_2$};  
		\end{scope}
